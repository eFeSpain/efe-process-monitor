...
func TestThreatScore(t *testing.T) {
	cases := []struct {
		...
		{"staging path", Conn{VT: "N/A", Suspicious: true}, 25, "staging"},
		{"downloads path scores less than staging",
			Conn{VT: "N/A", Untrusted: true}, 8, "descargas"},
		{"malware port (live tooling)", Conn{VT: "N/A", SuspPort: true, Known: "Metasploit"}, 12, "Metasploit"},
		{"bad spawn chain",
			Conn{VT: "N/A", Details: &ProcDetails{BadSpawn: "winword.exe → powershell.exe"}},
			40, "cadena anómala"},
